...
1. **开场（1min）**：业务闭环一句话 + 架构图（§2），强调「可解释」的匹配结果。
2. **请求链演示（5min）**：打开 `http://localhost:5173` → 点「加载示例」简历 → 搜索并选择岗位 → 点「开始匹配分析」→ 展示分数/命中/缺口/简历问题；同步用 `scripts/smoke_test.py` 演示接口层。
3. **核心算法（4min）**：讲清洗 → 归一化 → 打分公式（§6），指出「LLM 优先、规则兜底」的降级设计。
4. **工程化（3min）**：四层服务职责、snake_case 契约、H2 集成测试、Docker 编排、PDF 可选输入。
5. **取舍与扩展（2min）**：讲 §9 的取舍；可扩展方向 = 真实 LLM 简历优化建议、岗位推荐/倒排索引、分析历史落库。
